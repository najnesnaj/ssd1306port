...
void ssd1306_init(uint32_t dc, uint32_t rs, uint32_t cs, uint32_t clk, uint32_t mosi)
{
	...
	nrf_drv_spi_config_t const config =
	{
		.sck_pin  = clk,
		.mosi_pin = mosi,
		.miso_pin = 3, //hard coded, I know, but apparently not used ...
		.ss_pin   = cs,
	...
	//   nrf_drv_spi_init(&m_spi_master, &config, NULL);
...
void ssd1306_begin(uint8_t vccstate, uint8_t i2caddr, bool reset)
{
	...
	// Init sequence for 64x32 OLED module
	ssd1306_command(SSD1306_DISPLAYOFF);                    // 0xAE
	ssd1306_command(SSD1306_SETDISPLAYCLOCKDIV);            // 0xD5
	ssd1306_command(0x80);                                  // the suggested ratio 0x80
	ssd1306_command(SSD1306_SETMULTIPLEX);                  // 0xA8
	ssd1306_command(31); //display height - 1

	ssd1306_command(SETDISPLAYOFFSET);

	ssd1306_command(0x00);

	ssd1306_command(SETSTARTLINE);

	ssd1306_command(CHARGEPUMP);

	ssd1306_command(0x14);

	ssd1306_command(MEMORYMODE);

	ssd1306_command(0x00);

	ssd1306_command(SEGREMAP | 0x01);

	ssd1306_command(COMSCANDEC);  // changed from COMSCANINC
	...
	ssd1306_command(SETPRECHARGE);

	ssd1306_command(0x1F);             // changed from 0xF1

	ssd1306_command(SETVCOMDETECT);  // 0xDB

	ssd1306_command(0x40);

	ssd1306_command(DISPLAYALLON_RESUME);
	ssd1306_command(NORMALDISPLAY);

	ssd1306_command(DEACTIVATESCROLL);
